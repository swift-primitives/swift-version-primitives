...
extension Version.Semantic: Parseable {
    ...
    /// SemVer 2.0.0 byte-stream parser instantiation.
    ///
    ...
    /// existing nested generic ``Version/Semantic/Parser`` collides
    ...
    public typealias _ParseableParser = Version_Primitives.Version.Semantic.Parser<Parser_Primitives_Core.Parser.Input.Bytes>

    /// The canonical SemVer 2.0.0 byte-stream parser instance.
    ///
    /// Conforming to ``Parseable`` from `swift-parser-primitives`
    /// declares ``Version/Semantic/Parser`` (instantiated over
    ...
    ///
    /// ```swift
    /// let version = try Version.Semantic(ascii: Swift.Array("1.5.0".utf8))
    /// ```
    ///
    /// `init(ascii:)` does NOT assert input exhaustion — the parser
    /// is greedy over the SemVer character class and stops at the
    /// first non-SemVer byte; trailing bytes are silently dropped.
