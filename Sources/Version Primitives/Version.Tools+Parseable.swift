...
extension Version.Tools: Parseable {
    ...
    /// SE-0152 tools-version byte-stream parser instantiation.
    ///
    ...
    /// existing nested generic ``Version/Tools/Parser`` collides
    ...
    ///
    /// ```swift
    /// let version = try Version.Tools(ascii: Swift.Array("6.3".utf8))
    /// ```
    ///
    /// `init(ascii:)` does NOT assert input exhaustion — the parser
    /// is greedy over the tools-version character class and stops at
    /// the first non-matching byte; trailing bytes are silently
    /// dropped. For one-shot string parsing with non-ASCII detection
    /// AND trailing-bytes assertion baked in, use ``init(parsing:)`` —
